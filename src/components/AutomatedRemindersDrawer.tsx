...
export const AutomatedRemindersDrawer: React.FC<AutomatedRemindersDrawerProps> = ({
  ...
  const [testEmail, setTestEmail] = useState<string>('info@lebenswerk.praxismail.ch');
  const [reminderType, setReminderType] = useState<'reminder-24h' | 'reminder-2h' | 'booking-confirmed'>('booking-confirmed');
  ...
  const fetchDiagnostics = async () => {
    ...
    try {
      ...
      setDiagnostics(data);
      if (data.doctorNotificationEmail && data.doctorNotificationEmail !== "info@lebenswerk.praxismail.ch") {
        setTestEmail(data.doctorNotificationEmail);
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      ...
      >
        {/* Header */}
        <div className="bg-[#1B5E20] text-white px-6 py-4 flex items-center justify-between shrink-0 border-b border-white/10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 id="reminders-drawer-title" className="font-bold text-base font-serif">
                E-Mail-Benachrichtigungssystem & Hostpoint SMTP
              </h2>
              <p className="text-xs text-white/80">
                Automatische Benachrichtigung für Vigan Musliu bei neuen Patientenbuchungen & Terminerinnerungen
              </p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {/* Hostpoint Configuration Status Box */}
          <div className="bg-[#1B5E20]/5 border border-[#A5D6A7] rounded-2xl p-5 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm font-bold text-[#1B5E20]">
                <ShieldCheck className="w-5 h-5 text-[#1B5E20]" />
                <span>Hostpoint E-Mail-Konfiguration (Schweiz)</span>
              </div>
              <span className={`text-xs px-2.5 py-1 rounded-full font-bold ${
                ...
                {diagnostics?.activeProvider || 'Wird geprüft...'}
              </span>
            </div>

            <p className="text-xs text-[#2E7D32] leading-relaxed">
              Wenn ein Patient auf der Website einen Termin bucht, wird sofort eine detaillierte E-Mail mit Name, Telefon, Beschwerden und Google Kalender-Eintrag an das hinterlegte Hostpoint-Postfach versendet.
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs">
              <div className="bg-white p-3 rounded-xl border border-[#A5D6A7]">
                <span className="font-semibold text-slate-500 block text-[11px]">Hostpoint SMTP Server</span>
                <span className="font-bold text-[#1B5E20] font-mono">{diagnostics?.smtpHost || 'asmtp.mail.hostpoint.ch'}</span>
              </div>
              <div className="bg-white p-3 rounded-xl border border-[#A5D6A7]">
                <span className="font-semibold text-slate-500 block text-[11px]">Port / SSL</span>
                <span className="font-bold text-[#1B5E20] font-mono">Port {diagnostics?.smtpPort || '465'} (SSL)</span>
              </div>
              <div className="bg-white p-3 rounded-xl border border-[#A5D6A7]">
                <span className="font-semibold text-slate-500 block text-[11px]">Empfänger (Arzt/Praxis)</span>
                <span className="font-bold text-[#1B5E20] truncate block">{diagnostics?.doctorNotificationEmail || 'info@lebenswerk-physio.ch'}</span>
              </div>
            </div>
          </div>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-md ${
                    ...
                    {diagnostics?.activeProvider || 'Prüfe Status...'}
                  </span>
                </div>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {diagnostics?.smtpStatus === 'connected_and_verified' 
                    ? `Erfolgreich mit Hostpoint verbunden als ${diagnostics.maskedUser}. Benachrichtigungen gehen an ${diagnostics.doctorNotificationEmail}.`
                    : diagnostics?.smtpError 
                      ? `⚠️ Authentifizierungsfehler: ${diagnostics.smtpError}. Bitte Hostpoint-Passwort im Control Panel prüfen.`
                      : diagnostics?.hasSmtpUser 
                        ? `SMTP_USER geladen: ${diagnostics.maskedUser}. Verbindung wird getestet...`
                        : 'Noch keine Hostpoint Zugangsdaten im Secrets-Menü hinterlegt. Bitte SMTP_USER & SMTP_PASS eintragen.'}
                </p>
              </div>
            </div>
          ...
          <div className="bg-white border border-[#A5D6A7] p-5 rounded-3xl">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-[#1B5E20] flex items-center gap-1.5">
                <Send className="w-3.5 h-3.5 text-[#1B5E20]" />
                Live Test-E-Mail an Hostpoint-Konto senden
              </h3>
              {testSentNotice && (
            ...
            <form onSubmit={handleSendTestReminder} className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
              <div className="sm:col-span-4">
                <input
                  ...
                  placeholder="vigan@lebenswerk-physio.ch"
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-xs focus:ring-2 focus:ring-[#1B5E20] focus:outline-none"
                />
              </div>
              ...
              <div className="sm:col-span-3">
                <select
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-xs font-medium focus:ring-2 focus:ring-[#1B5E20] focus:outline-none"
                >
                  <option value="booking-confirmed">Buchungsbestätigung</option>
                  <option value="reminder-24h">24h Terminerinnerung</option>
                  <option value="reminder-2h">2h Terminerinnerung</option>
                </select>
              </div>
              ...
              <div className="sm:col-span-2">
                <button
                  ...
                  title="Erstellt eine Test-Vorschau in der internen Liste"
              ...
              <div className="sm:col-span-3">
                <button
                  ...
                  title="Sendet eine echte E-Mail über Hostpoint SMTP an Ihre Mailbox"
                  className="w-full py-2.5 px-3 rounded-xl bg-[#1B5E20] hover:bg-[#144818] text-white font-semibold text-xs shadow-md flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 transition-all"
                >
                  {isSendingRealEmail ? (
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                  ) : (
        ...
        <div className="bg-white border-t border-slate-200 px-6 py-3.5 flex items-center justify-between text-xs text-slate-500">
          <span className="flex items-center gap-1.5 text-[#1B5E20] font-medium">
            <ShieldCheck className="w-4 h-4 text-[#1B5E20]" /> Hostpoint SMTP & SSL-verschlüsselte Datenübertragung aktiv
          </span>
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-[#1B5E20] hover:bg-[#144818] text-white font-semibold cursor-pointer shadow-xs transition-colors"
          >
            Schliessen
          </button>
        </div>
